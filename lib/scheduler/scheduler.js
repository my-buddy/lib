...
class ScheduleEvent{
    constructor(options){
        options = options || {};
        options = _.merge({enter: null, exit: null}, options);
...
class Scheduler{
    ...
    subscribe(schedule, cb){
        ...
        // Moment schedule
            ...
            if(!schedule.hour && !schedule.date){
    ...
    _subscribeMoment(schedule, cb){
        ...
        // Subscribe a date schedule
        if(schedule.date instanceof Date){
            schedule.date.setSeconds(schedule.date.getSeconds() + 1);
            var job = nodeSchedule.scheduleJob(schedule.date, function(){
                return cb(new ScheduleEvent());
            });
            //console.log(require('util').inspect(job));
            //console.log(job.nextInvocation());
            scheduleProcess.nextTick = 'todo'; // @todo
            scheduleProcess.nextInvocation = job.nextInvocation;
            return scheduleProcess;
        }

        // Subscribe recurrence rule
        var rule = new nodeSchedule.RecurrenceRule();
        rule.hour = schedule.hour || null;
        rule.minute = schedule.minute || null;
        rule.seconds = schedule.seconds || 0;
        console.log(rule);
        var job = nodeSchedule.scheduleJob(rule, function(){
            return cb(new ScheduleEvent());
        });
